                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>实验室类型</label>
                                        <input id="shiyanshiValue" name="shiyanshiValue" class="form-control"
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="shiyanshiId" name="shiyanshiId" type="hidden">
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>人数</label>
                                        <input id="shiyanshiYuyueNumber" name="shiyanshiYuyueNumber" class="form-control"
                                               onchange="shiyanshiYuyueNumberChickValue(this)"  placeholder="人数">
                                    </div>

                                    <div class="form-group  col-md-6">
                                        <label>申请详情</label>
                                        <input id="shenqingContentupload" name="file" type="file">
                                    ...
                                    <div class="form-group col-md-6 aaaaaa">
                                        <label>是否同意</label>
                                        <select id="shiyanshiYuyueTypesSelect" name="shiyanshiYuyueTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#shenqingContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('shenqingContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("shiyanshiYuyue/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addshiyanshiYuyue');
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationShiyanshiyuyuetypesSelect(){
            var shiyanshiYuyueTypesSelect = document.getElementById('shiyanshiYuyueTypesSelect');
            if(shiyanshiYuyueTypesSelect != null && shiyanshiYuyueTypesOptions != null  && shiyanshiYuyueTypesOptions.length > 0 ){
                for (var i = 0; i < shiyanshiYuyueTypesOptions.length; i++) {
                    shiyanshiYuyueTypesSelect.add(new Option(shiyanshiYuyueTypesOptions[i].indexName,shiyanshiYuyueTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(shiyanshiYuyueTypesSelect != null && shiyanshiYuyueTypesOptions != null  && shiyanshiYuyueTypesOptions.length > 0 ) {
            for (var i = 0; i < shiyanshiYuyueTypesOptions.length; i++) {
                if (shiyanshiYuyueTypesOptions[i].codeIndex == ruleForm.shiyanshiYuyueTypes) {//下拉框value对比,如果一致就赋值汉字
                        shiyanshiYuyueTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.shenqingContent != null && ruleForm.shenqingContent != 'null' && ruleForm.shenqingContent != '' && $("#shenqingContentupload").length>0) {

            var shenqingContentUeditor = UE.getEditor('shenqingContentEditor');
            shenqingContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.shenqingContent != null){
                    mes = ''+ ruleForm.shenqingContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#shenqingContentupload").length>0) {
            var shenqingContentEditor = UE.getEditor('shenqingContentEditor');
            if (shenqingContentEditor.hasContents()) {
                $('#shenqingContent-input').attr('value', shenqingContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                yuyueTime: "required",
                insertTime: "required",
                shiyanshiYuyueNumber: "required",
                shenqingContent: "required",
                shiyanshiYuyueTypes: "required",
            },
            messages: {
                yonghuId: "学生不能为空",
                shiyanshiId: "实验室不能为空",
                yuyueTime: "预约时间不能为空",
                insertTime: "申请时间不能为空",
                shiyanshiYuyueNumber: "人数不能为空",
                shenqingContent: "申请详情不能为空",
                shiyanshiYuyueTypes: "是否同意不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addshiyanshiYuyue = window.sessionStorage.getItem("addshiyanshiYuyue");
        if (addshiyanshiYuyue != null && addshiyanshiYuyue != "" && addshiyanshiYuyue != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("shiyanshiYuyue/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#shiyanshiYuyueNumber").val(ruleForm.shiyanshiYuyueNumber);
        $("#shenqingContent").val(ruleForm.shenqingContent);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationShiyanshiyuyuetypesSelect();
            <!--  初始化级联表的下拉框  -->
